...
chrstat <-chrstat[,c("chr","lengthadj", "chrmid")]

genepos<-read.table(file="./rn6_gene_pos.tab", sep="\t", header=F)
names(genepos)<-c("gene","chr","start","end")
genepos<-merge(genepos,chrlength[,c("chr","lengthadj")], by="chr")
...
head(df0)
dim(df0)
df1<-merge(df0,chrlength,by="chr")
...
names(all_data)
dat<-all_data[,c("gene","chr","region","bp","rn6_start","rn6_end", "cistrans", "logp")]
save(file="eqtl.Rdata", dat)
